...
public class UpdateDetails extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			con = DriverManager.getConnection("jdbc:mysql://localhost:3306/fv","root","");
			st = con.createStatement();
			
			DBConnect connect = new DBConnect();
			String sql = "UPDATE `oder` SET `customerName`='"+Customer_name+"',`address`='"+Customer_address+"',`telephoneNo.`='"+Customer_contactNo+"',`items`='"+Customer_Items+"',"
					+ "`amount`='"+Customer_amount+"',`deliveryCharges`='"+Customer_deliveryCharges+"',`finalAmount`='"+Customer_finalAmount+"',`deliveryStatus`='Pending' WHERE `oderNo:`='"+Customer_oder_no+"'";
